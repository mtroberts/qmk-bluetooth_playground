...
#    ifndef COMBO_ENABLE
#        define IS_TAPPING_RECORD(r) (IS_EVENT(tapping_key.event) && KEYEQ(tapping_key.event.key, (r->event.key)))
#    else
#        define IS_TAPPING_RECORD(r) (IS_EVENT(tapping_key.event) && KEYEQ(tapping_key.event.key, (r->event.key)) && tapping_key.keycode == r->keycode)
#    endif
...
#    endif // defined(CHORDAL_HOLD) || defined(FLOW_TAP_TERM)
    ...
    // if tapping
    if (IS_EVENT(tapping_key.event) && tapping_key.event.pressed) {
...
#    endif
    ...
    } else if (IS_EVENT(tapping_key.event) && !tapping_key.event.pressed) {
        if (WITHIN_TAPPING_TERM(event) || MAYBE_RETRO_SHIFTING(event, keyp)) {
            if (IS_NOEVENT(event)) {
                // early return for tick events
                return true;
